...
def handle_edit_text(user_id: str, text: str):
    ...
    if draft.step == "title":
        if not text:
            return TextSendMessage(text="タイトルを入力してね")
        draft.name = text; draft.step = "menu"; draft.save()
    ...
    if draft.step == "start_time":
        ...
        draft.start_time = new_dt; draft.start_time_has_clock = True; draft.step = "menu"; draft.save()
        return ui.ask_edit_menu()
    ...
    if draft.step == "end_time":
        ...
        draft.end_time = new_dt; draft.end_time_has_clock = True; draft.step = "menu"; draft.save()
        return ui.ask_edit_menu()
    ...
    if draft.step == "duration":
        ...
        draft.end_time = draft.start_time + delta; draft.end_time_has_clock = False; draft.step = "menu"; draft.save()
        return ui.ask_edit_menu()
    ...
    if draft.step == "cap":
        ...
        draft.capacity = capacity; draft.step = "menu"; draft.save()
        return ui.ask_edit_menu()
...
def handle_edit_postback(user_id: str, scope_id: str, data: str, params: dict):
    ...
    if data == "edit=start_date":
        draft.step = "start_date"; draft.save()
        return ui.ask_date_picker("新しい日付を選んでね", data="pick=start_date", with_back=True, with_reset=True)
    ...
    if m:
        ...
        if kind == "start" and draft.step == "start_time":
            if v == "__skip__":
                draft.start_time_has_clock = False; draft.step = "menu"; draft.save()
                return ui.ask_edit_menu()
    ...
    if data == "endmode=enddt":
        ...
        draft.end_time_has_clock = False; draft.step = "end_time"; draft.save()
        return ui.ask_time_menu("終了時刻を HH:MM で入力するか、下から選んでね", prefix="end")

    if data == "endmode=duration":
        draft.end_time_has_clock = False; draft.step = "duration"; draft.save()
        return ui.ask_duration_menu()

    if data == "endmode=skip":
        draft.end_time = None; draft.end_time_has_clock = False; draft.step = "menu"; draft.save()
        return ui.ask_edit_menu()
